...
TEMP=$(sensors | grep Tctl | awk '{print $2}' | cut -d '+' -f 2 | cut -d "." -f 1)
    ...
    if [ $TEMP -ge 94 ]; then
        sh -c "echo 0 > /sys/devices/platform/asus-nb-wmi/hwmon/hwmon6/pwm1_enable"
        sh -c "echo 0 > /sys/devices/platform/asus-nb-wmi/hwmon/hwmon7/pwm1_enable"
    ...
    if [ $TEMP -lt 80 ];then
        if [ $FAN == 1 ]; then
            sh -c "echo 2 > /sys/devices/platform/asus-nb-wmi/hwmon/hwmon6/pwm1_enable"
            sh -c "echo 2 > /sys/devices/platform/asus-nb-wmi/hwmon/hwmon7/pwm1_enable"
